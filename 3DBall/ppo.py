...
class PPO():
    ...
    def update(self):
        ...
        reward = (reward - reward.mean()) / (reward.std() + 1e-10)  # reward归一化，更好收敛
        with torch.no_grad():
            target_v = reward + args.gamma * self.critic_net(next_state)

        advantage = (target_v - self.critic_net(state)).detach()
        for _ in range(self.ppo_epoch):  # iteration ppo_epoch
            for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity), self.batch_size, True)):#抽取一个batch
