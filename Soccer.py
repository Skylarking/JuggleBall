...
########################### step1.创建环境和打印环境信息 ########################
EnvName = 'StrikerVsGoalie_Linux'
full_env = './linux_env/StrikerVsGoalie/SvsG_Linux.x86_64'
...
env = UnityEnvironment(file_name=full_env, seed=1, side_channels=[channel])
...
############################ step3.加载模型 ###############################
if not os.path.exists('model_ma_entropy'): os.mkdir('model_ma_entropy')
file_path = "./model_ma_entropy/"
...
''' 运行流程 '''
# 参数
opt = SoccerParams.parms()
kwargs = {
            "n_agents": n_agents,
            "state_dim": state_dim_lst,
            "action_dim": action_dim_lst,
            "gamma": opt.gamma,
            ...
            "optim_batch_size":opt.optim_batch_size,
...
while total_steps < Max_train_steps:
    ...
    while not done:
        ...
        # dw: 如果完成任务或者死亡，但是没到最大步数，在计算adv和td target时用dw，而不是最大步数结束时的done
        ...
        # train
        data = (s, a, reward, s_next, prob_n, done, dw)
        model.put_data(data)
