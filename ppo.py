...
class GaussianActor_musigma(nn.Module):
	...
	def forward(self, state):
		...
		sigma = F.softplus( self.sigma_head(a) ) + 0.01
		return mu,sigma
...
class CentralizedCritic(nn.Module):
	def __init__(self, state_dim, net_width):
		...
		self.C1 = nn.Linear(state_dim, net_width)
		self.C2 = nn.Linear(net_width, net_width)
		self.C3 = nn.Linear(net_width, net_width)
		self.C4 = nn.Linear(net_width, 1)

	def forward(self, state):
		v = torch.tanh(self.C1(state))
		v = torch.tanh(self.C2(v))
		v = torch.tanh(self.C3(v))
		v = self.C4(v)
		return v
...
class PPO(object):
	...
	def train(self):
		...
		for i in range(self.K_epochs):
			...
			for i in range(a_optim_iter_num):
				index = slice(i * self.a_optim_batch_size, min((i + 1) * self.a_optim_batch_size, s.shape[0]))	# minibatch的切片
				distribution = self.actor.get_dist(s[index])
				dist_entropy = distribution.entropy().sum(1, keepdim=True)
				logprob_a_now = distribution.log_prob(a[index])
				old = logprob_a[index]
				ratio = torch.exp(logprob_a_now.sum(1,keepdim=True) - old.sum(1,keepdim=True))  # a/b == exp(log(a)-log(b))
	...
	def make_batch(self):
		...
		'''list to tensor'''
		with torch.no_grad():
			s, a, r, s_prime, logprob_a, done_mask = \
	...
	def save(self,episode, file_path="./model/"):
		torch.save(self.critic.state_dict(), file_path + "ppo_critic{}.pth".format(episode))
		torch.save(self.actor.state_dict(), file_path + "ppo_actor{}.pth".format(episode))


	def load(self ,episode, file_path="./model/"):
		self.critic.load_state_dict(torch.load(file_path + "ppo_critic{}.pth".format(episode)))
		self.actor.load_state_dict(torch.load(file_path + "ppo_actor{}.pth".format(episode)))
